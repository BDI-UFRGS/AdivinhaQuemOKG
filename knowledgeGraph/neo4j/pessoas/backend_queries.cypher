...
WHERE type(relGF) IN ['TEM', 'TEM_TIPO']
  AND (relGF.pessoa IS NULL OR relGF.pessoa = '')
WITH DISTINCT feature, grupo
OPTIONAL MATCH path = (feature)-[:TEM|TEM_TIPO*0..2]->(contextCat:Categoria)
WHERE ALL(rel IN relationships(path) WHERE rel.pessoa IS NULL OR rel.pessoa = '')
OPTIONAL MATCH (contextCat)-[atrib:TEM_TIPO]->(:Valor)
WHERE atrib.pessoa IS NOT NULL
WITH feature,
     grupo,
...
WHERE atribContext.pessoa IS NOT NULL
...
WHERE type(relFA) IN ['TEM', 'TEM_TIPO']
  AND (relFA.pessoa IS NULL OR relFA.pessoa = '')
  AND agg <> feature
...
WHERE NOT EXISTS {
  ...
  WHERE type(relChild) IN ['TEM', 'TEM_TIPO']
    AND (relChild.pessoa IS NULL OR relChild.pessoa = '')
    AND child <> feature
}
RETURN feature.id AS featureId,
       featureSlug,
...
WITH collect(entity) AS pessoas
CALL {
  WITH pessoas
  UNWIND pessoas AS entity
