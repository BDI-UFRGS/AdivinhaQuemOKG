DROP CONSTRAINT diagnostico_nome IF EXISTS;
DROP CONSTRAINT categoria_nome IF EXISTS;
DROP CONSTRAINT valor_nome IF EXISTS;
...
CREATE INDEX idx_diag_nome IF NOT EXISTS FOR (d:Diagnostico) ON (d.nome);
CREATE INDEX idx_cat_nome  IF NOT EXISTS FOR (c:Categoria)   ON (c.nome);
CREATE INDEX idx_val_nome  IF NOT EXISTS FOR (v:Valor)       ON (v.nome);
